...
export function mount(props) {
	createDomElement(props.customProps.DOMSelector);
...
function createDomElement(DOMSelector) {
	// Make sure there is a div for us to render into
	let el = document.querySelector(DOMSelector);
	if (!el) {
		el = window.document.createElement('dashboard-form');
		el.id = 'dashboard-form';
